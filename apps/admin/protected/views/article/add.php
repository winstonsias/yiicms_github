...
<block name="body">
	...
	<div class="main-title cf">
		<h2>
			新增<?php echo get_document_model($info['model_id'],'title')?><?php if(is_array($this->assign['rightNav'])){?>[
...
<div class="tab-wrap">
	...
	<!-- 表单 -->
	<form id="form" action="<?php echo url('article/add')?>" method="post" class="form-horizontal">
		<!-- 基础文档模型 -->
		<?php foreach (parse_config_attr($model['field_group']) as $key=>$group){?>
        ...
        <div id="tab<?php echo $key?>" class="tab-pane <?php echo $key==1?'in':''?> tab<?php echo $key?>">
            ...
            <?php if($field['is_show']==1||$field['is_show']==2){?>
                ...
                <div class="form-item cf">
                    ...
                    <div class="controls">
                    <?php get_html_by_field_type($field,null,gp('cate_id'));?>
		...
		<div class="form-item cf">
			...
			<a class="btn btn-return" href="<?php echo url('article/index',array('cate_id'=>gp('cate_id')))?>">返 回</a>
			<?php if(C('OPEN_DRAFTBOX')&&(strtolower($this->getAction()->getId())=='add'||$info['status']==3)){?>
			...
			<input type="hidden" name="category_id" value="<?php echo isset($info['category_id'])?$info['category_id']:''?>">
		</div>
	</form>
	</div>
</div>
